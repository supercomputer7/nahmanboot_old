...
void parseELF(Elf32MainHeader* main_header,uint32_t section_data)
{
    ...
    uint8_t* buf4 = (uint8_t*)(section_data);
...
void parseELF64(Elf64MainHeader* main_header,uint32_t section_data)
{
    Elf64ProgramHeader* progs = (Elf32ProgramHeader*)((uint32_t)main_header + (main_header->program_header_table_position));
    for(int i=0; i < main_header->program_header_table_entries_count ; ++i)
    {
        // zeroing p_memsz bytes at p_vaddr
        uint8_t* buf = (uint8_t*)(progs[i].p_vaddr);
